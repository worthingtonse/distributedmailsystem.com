...
const SuccessPage = () => {
  ...
  const [copiedAddress, setCopiedAddress] = useState(false); // Renamed to avoid conflict
  const [copiedReferral, setCopiedReferral] = useState(false);

  const generatedAddress = state?.generatedAddress || 'Address.Not.Found#000';
  const recipientName = state?.recipientName || 'User';

  const referralLink = `https://distributedmailsystem.com/ref/qmailuser`; // Generic referral link

  // Social Sharing Functions (using copied from FunnelSuccess for referralLink)
  const shareOnX = () => {
  ...
  // New handler for copying the generated address
  const handleCopyGeneratedAddress = () => {
        ...
        <div className="lg:col-span-8 space-y-8 text-left">
          ...
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="bg-black/40 backdrop-blur-3xl p-8 rounded-[32px] border border-white/10 shadow-2xl">
            <div className="flex items-center gap-5 mb-4">
               ...
               <div>
                  <h1 className="text-3xl font-black text-white uppercase tracking-tight">Address Minted!</h1>
                  <p className="text-gray-400 leading-tight">Your secure Distributed Mail address is ready.</p>
               </div>
            </div>
            {/* Display and Copy Address */}
            <div className="bg-black/80 p-6 rounded-2xl border border-gray-800 relative group mt-4">
              <code className="text-xl md:text-2xl font-mono text-blue-400 break-all">
                {generatedAddress}
              </code>
              <button 
                onClick={handleCopyGeneratedAddress}
              ...
              >
                {copiedAddress ? <Check size={18}/> : <Share2 size={18}/>}
              </button>
            </div>
            ...
            <div className="space-y-6">
              <div className="p-5 bg-black/60 rounded-2xl border border-white/5">
                ...
                <div className="flex items-center gap-3">
                   ...
                   >
                     {copiedReferral ? <Check size={16} className="text-green-400" /> : <Share2 size={16}/>}
                   </button>
                </div>
                {copiedReferral && (
                  <motion.div 
